...
public class CityRepositoryImpl {
    ...
    public List<City> findAll(){
        Query query = entityManager.createQuery("select c from City c left join fetch c.regions");
        return query.getResultList();
    }

    public List<City> findById(Integer id) {
        Query query = entityManager.createQuery("select c from City c left join fetch c.regions where c.id = :id");
